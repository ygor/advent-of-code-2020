...
p (other_tickets-valid_tickets).map { |t| t.select { |n| !all_ranges.any? {|r| r.include?(n) } } }.flatten.inject(0){|sum,x| sum + x }
...
p ticket_headers.map.with_index {|h,i| h.start_with?('departure') ? i : -1 }.select {|i| i >= 0 }.inspect
